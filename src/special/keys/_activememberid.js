function determineActiveValue(member, roles) {
  var {
    messages = 0,
    highestroleorder = 0,
    lastmessage = 0,
  ...
  var messageWeight = Math.pow(roundTo(messages, 50) * highestroleorder / roles * 5, 1.5)
...
module.exports = {
  ...
  func: function (msg) {
    ...
    var weightedMembers = activeMembers.map(([id, member]) => {
      var weight = determineActiveValue(member, roles)
